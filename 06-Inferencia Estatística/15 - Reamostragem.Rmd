...
- O jackknife é uma ferramenta para estimar os erros-padrão eo viés dos estimadores
...
- Ambos os métodos (naturalmente) rendem um viés estimado de `r temp$jack.bias` e um se de` r temp$jack.se`
...
- Geralmente não use o jackknife para quantiles amostra como a mediana; Como se mostrou ter algumas propriedades pobres
...
- Melhor percentil bootstrap intervalos de confiança correto para viés
...
- Um exemplo estatístico seria a diferença nas médias entre os dois grupos; pode-se também usar uma estatística t
