...
## Análise bayesiana
- Estatísticas bayesianas postulam a * prior * sobre o parâmetro de interesse
- Todas as inferências são então realizadas na distribuição de o parâmetro dado os dados, chamado de posterior
- Em geral,
  $$
  \mbox{A Posteriori} \propto \mbox{Prtobabilidade} \times \mbox{A Priori}
  $$
- Portanto, como vimos nos testes diagnósticos, a o fator pelo qual nossas crenças prévias são atualizadas para produzir conclusões à luz dos dados recolhidos

---
## Especificação prévia
- A distribuição beta é a prioridade prévia para parâmetros entre $0$ e $1$.
...
- A média posterior é uma mistura do MLE ($\hat p$) e do média anterior
- $\pi$ vai para $1$ como $n$ fica grande; Para grandes $n$ os dados pântanos do anterior
- Para pequenos $n$, a média anterior domina
- Generaliza como a ciência idealmente deveria funcionar; À medida que os dados se tornam cada vez mais disponíveis, as crenças prévias deveriam impor-
- Com um anterior que é degenerado em um valor, nenhuma quantidade de dados pode superar o anterior
...
$$
- O "prior de Jeffrey", que tem alguns benefícios teóricos coloca $\alpha = \beta = .5$
...
- A $95\%$ crível intervalo, $[a, b]$ iria satisfazer
  $$
  P(p \in [a, b] ~|~ x) = .95
  $$
...
## Obtendo intervalos HPD para este exemplo
...
Dá o intervalo HPD.
- O nível credível padrão é $95\%$ e O anterior padrão é o anterior do Jeffrey.
