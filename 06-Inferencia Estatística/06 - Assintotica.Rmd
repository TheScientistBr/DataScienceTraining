...
* Os tipos de asymptotics que fazem são ordens de magnitude mais difíceis de trabalhar com
...
$$
- Substituindo $p$ por $\hat p$ no erro padrão resulta no que é chamado de intervalo de confiança Wald para $p$
- Observe também que $p(1-p) \leq 1/4$ para $0 \leq p \leq 1 $
- Vamos $\alpha = .05$ para que $z_{1 - \alpha / 2} = 1.96 \approx 2$ então
...
* `1 / sqrt (100) =. 1` assim um back do cálculo do envelope dá um intervalo aproximado de 95% de `(0.46, 0.66)`
* Não é suficiente para você relaxar, melhor ir fazer mais campanhas!
* Diretrizes aproximadas, 100 para 1 casa decimal, 10.000 para 2, 1.000.000 para 
